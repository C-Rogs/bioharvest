...
2. Allow Health. Missing samples stay JSON `null`.
...
HealthKit read only. `ExportPayload.currentSchemaVersion` is 2, `app` is `bioharvest`, `purpose` is `time_series_coach_export`. Share writes `latest_export.json` into App Group `group.com.cameronro.coach` and opens `coach://import/latest`. Settings can POST the same bytes to a webhook.
